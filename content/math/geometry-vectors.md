...
- 圓與直線關係（circle-line relation）: 利用圓心到直線距離 $d$ 與 $r$ 比較
  ...
  - $d = r$ 相切（一交點）
  - $d > r$ 相離（無交點）
- 💡 聯想：GPS 定位原理就是「三個圓的交點」，衛星到你的距離各畫一個圓，交點就是你的位置
- 💡 聯想：點到直線距離公式就像量「你家到馬路」的最短距離——一定是垂直的那一段
### 線性規劃
- 可行解區域（feasible region）: 滿足所有限制條件的點所形成的區域
  - 由多個二元一次不等式的交集構成
  - 通常是一個凸多邊形
- 目標函數（objective function）: $f(x, y) = ax + by$，求最大值或最小值
- 頂點定理（vertex theorem）: 目標函數的最佳值必出現在可行解區域的頂點上
  - 解法: 逐一計算各頂點的函數值，取最大或最小
- 💡 聯想：工廠要用有限原料做兩種產品，怎麼分配利潤最高——這就是線性規劃
...
- 💡 聯想：拔河比賽兩隊的拉力合成就是向量加法，誰贏取決於合力方向
- 💡 聯想：物理的「分力」就是向量分解——斜面上的重力分成沿斜面和垂直斜面兩個分量
...
- 💡 聯想：VR 遊戲裡角色的位置 $(x,y,z)$ 和視線方向就是空間向量的應用
- 💡 聯想：法向量就像桌面上立著的筆——垂直於整個桌面，只要知道法向量就能確定平面方向
